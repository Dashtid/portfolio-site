...
class ScrollManager {
  ...
  initSmoothScrolling() {
    ...
    const internalNavLinks = document.querySelectorAll('.internal-nav[data-scroll]');
  ...
  setupNavigationHighlighting() {
    ...
    const navLinks = document.querySelectorAll('.internal-nav[data-scroll]');

    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const currentNavLink = document.querySelector(`[data-scroll="${entry.target.id}"]`);
          if (currentNavLink) {
...
class ProjectManager {
  constructor() {
    ...
    this.categories = {
      'cybersecurity': ['security', 'cyber', 'auth', 'encryption', 'vulnerability', 'pentest'],
      'healthcare': ['medical', 'health', 'hospital', 'patient', 'dicom', 'pacs'],
      'automation': ['script', 'automation', 'deploy', 'ci', 'cd', 'pipeline', 'workflow'],
