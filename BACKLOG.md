...
- **DEAD-07b** (LOW, ✅ done 2026-06-11): deleted `get_project_stats`/`get_repo_details`/`get_repo_commits` from `github_service.py` (~80 LOC) + 18 tests (`TestGetRepoDetails`, `TestGetRepoCommits`, `TestGetProjectStats`, `TestGetRepoCommitsEdgeCases`, `TestGetProjectStatsIntegration`, `TestLinkHeaderEdgeCases`, three `*_method_exists` scaffolding tests, plus two HTTPStatusError tests in `TestServiceHTTPErrors`) — ~394 LOC test removal. Also dropped now-unused `re` / `datetime` / `timedelta` / `UTC` imports. 750 backend tests pass at 85.24% coverage.
...
- **INFRA-002** (HIGH, ✅ done 2026-06-11): Portfolio data fetch moved from `onMounted` to top-level `await` in `setup()` of `HomeView.vue`. vite-ssg's renderer respects the existing `<Suspense>` boundary in `App.vue`, awaits the async setup, and serializes the populated Pinia state into `__INITIAL_STATE__`. **Verified post-build:** all four arrays (`companies`, `projects`, `skills`, `education`) populated in `dist/index.html`. Client-side hydration guard (`if (portfolioStore.companies.length === 0)`) short-circuits the duplicate fetch on the normal SSG-hydration path. Failures swallowed (logger.error only) so a transient API blip doesn't fail the SSG build. Dead `loading` ref dropped while in the area. (Tried `onServerPrefetch` first — vite-ssg doesn't await that hook, so it doesn't populate state; top-level await is the right pattern.) `tests/unit/views/HomeView.spec.ts` updated to wrap HomeView in `<Suspense>` harness, pre-populate Pinia with a sentinel, and stub `ThreeHeroBackground` (defineAsyncComponent that fails to load under happy-dom and breaks the surrounding tree). 559 frontend unit tests pass; type-check clean.
- **INFRA-003** (MEDIUM, ✅ done 2026-06-12): Backend API migrated from `dashti-portfolio-backend.fly.dev` to `api.dashti.se`. Loopia DNS (A `66.241.125.236` + AAAA `2a09:8280:1::aa:714e:0`) → Fly Let's Encrypt cert (verified `Issued: rsa,ecdsa`). Cookie attrs flipped from `SameSite=None+Secure` to `SameSite=Lax+Secure` since frontend and backend now share eTLD+1. Frontend `apiUrl` default and SSG fallback flipped to `https://api.dashti.se` (both `config/index.ts` and `main.ts`); `index.html` preconnect/dns-prefetch repointed; Sentry `tracePropagationTargets` regex updated. Vercel `/api/*` rewrite removed (frontend now hits api.dashti.se directly); Vercel + backend CSP `connect-src` cleaned of `*.fly.dev`. Docs (`DEPLOYMENT.md`, `SECURITY.md`), scripts (`verify-deployment.sh`), and CI environment URL repointed. Fly app NAME (`dashti-portfolio-backend`) stays — that's internal Fly routing, not user-facing.
