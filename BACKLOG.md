...
- **INFRA-002** (HIGH, **diagnosed 2026-06-11; root cause is HomeView data fetch, not the API**): Re-checked the live site after the Sprint 1-6 push. Backend API at `https://dashti-portfolio-backend.fly.dev/api/v1/companies/` returns real data (Hermes Medical Solutions + 7 others); the Vercel rewrite at `https://dashti.se/api/v1/companies/` also returns real data. But the SSG-rendered `__INITIAL_STATE__` still ships `"companies":[],"skills":[],"projects":[],"education":[]`. **Root cause:** `HomeView.vue` fetches via `portfolioStore.fetchAllData()` inside `onMounted`, which is fire-and-forget during SSG pre-render — vite-ssg serializes the Pinia state BEFORE the async fetch resolves. The visible page content comes from the static fallback (CONTENT-001 fix), so SEO and user-facing rendering are fine; only the client-hydration optimization is missing. **Fix requires:** refactoring HomeView's data fetch to a top-level `await` in `setup()` (or vite-ssg's `onSSRAppRendered` hook) so the SSG renderer waits for the fetch before serializing. Non-trivial — not blocking; deferred until prioritized.
