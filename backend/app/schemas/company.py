...
class CompanyCreate(CompanyBase):
    # Curated CV bullets — write-only on purpose. They live on CompanyCreate /
    # CompanyUpdate but NOT on CompanyBase, because CompanyResponse inherits
    # Base and GET /api/v1/companies/ is a PUBLIC endpoint: putting them there
    # published the printed CV's wording on the open API. The CV export reads
    # the model directly, so nothing needs them on the read schema.
