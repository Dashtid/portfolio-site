  ...
  try {
    ...
    for (const file of htmlFiles) {
      ...
      for (const script of inlineScripts) {
        if (!script.includes('nonce=') && !script.includes('type="application/ld+json"')) {
      ...
      const externalLinks = content.match(/<link[^>]*href="https?:\/\/[^"]*"[^>]*>/g) || []
      const externalScripts = content.match(/<script[^>]*src="https?:\/\/[^"]*"[^>]*>/g) || []
      
      for (const link of externalLinks) {
        if (!link.includes('integrity=')) {
          warnings.push(`External resource without SRI in ${file}: ${link.match(/href="([^"]*)"/)?.[1]}`)
        }
      }
      
      for (const script of externalScripts) {
        if (!script.includes('integrity=')) {
          warnings.push(`External script without SRI in ${file}: ${script.match(/src="([^"]*)"/)?.[1]}`)
        }
      }
    }
      ...
      if (pkg.dependencies) {
        ...
        for (const devPkg of devOnlyPackages) {
          if (pkg.dependencies[devPkg]) {
            warnings.push(`Dev dependency ${devPkg} found in production dependencies`)
          }
        }
      }
    }
