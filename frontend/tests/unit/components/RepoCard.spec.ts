...
describe('RepoCard', () => {
  ...
  it('renders stars and forks counts', () => {
    const wrapper = mount(RepoCard, { props: { repo: sampleRepo } })
    const stats = wrapper.findAll('.repo-stat')
    expect(stats).toHaveLength(2)
    expect(stats[0].text()).toContain('50')
    expect(stats[1].text()).toContain('5')
  })

  it('opens GitHub in a new tab with secure rel attributes', () => {
    const wrapper = mount(RepoCard, { props: { repo: sampleRepo } })
    const link = wrapper.find('a.repo-card-enhanced')
  ...
  it('paints the language dot with the linguist color for known languages', () => {
    const wrapper = mount(RepoCard, { props: { repo: sampleRepo } })
    const dot = wrapper.find('.language-dot')
    // TypeScript -> #3178c6
  ...
  it('omits the description when null', () => {
    ...
    expect(wrapper.find('.project-description').exists()).toBe(false)
  })
  ...
  it('omits the language pill when language is null', () => {
    ...
    expect(wrapper.find('.repo-language').exists()).toBe(false)
  })
})
